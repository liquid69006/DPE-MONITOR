...
def recuperer_dpe_zone(zone_cfg: dict, date_depuis: str) -> list:
    """
    Récupère les DPE en filtrant par code postal via l'URL encodée manuellement.
    Filtre ensuite par date côté Python.
    """
    ...
    for cp in codes:
        page = 0
        while True:
            # Encodage manuel pour éviter les problèmes avec requests et les parenthèses
            url = (
                f"{API_BASE}"
                f"?size=100"
                f"&page={page}"
            ...
            headers = {
                "User-Agent": "Mozilla/5.0 (compatible; DPE-Monitor/1.0)"
            }
            try:
            ...
            for dpe in resultats:
                date_str = dpe.get("date_reception_dpe") or dpe.get("date_reception_dpe", "")
